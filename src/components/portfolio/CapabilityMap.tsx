...
import { capabilityAreas, supportingCapabilities } from "./data";
...
export function CapabilityMap() {
  ...
  const activeArea =
    capabilityAreas.find((area) => area.id === activeId) ?? capabilityAreas[0];
      ...
      <div className="mx-auto max-w-6xl px-5 py-20 sm:px-8 md:py-28">
        <Reveal>
          <SectionIntro
            ...
            copy="Select an area to see the stack and product result it supports."
          />
        </Reveal>
        <Reveal delay={0.06}>
          <div className="grid gap-4 lg:grid-cols-[0.85fr_1.15fr]">
              ...
              {capabilityAreas.map((area) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <p
                      className={`mt-10 font-display text-xl tracking-[-0.035em] ${isActive ? "text-cyan" : "text-slate-100"}`}
                    >
                      {area.label}
                    </p>
            ...
            <motion.div
              ...
              role="tabpanel"
              aria-labelledby={`${activeArea.id}-tab`}
              initial={reducedMotion ? false : { opacity: 0, x: 12 }}
              animate={reducedMotion ? undefined : { opacity: 1, x: 0 }}
              transition={{ duration: 0.28 }}
              className="relative overflow-hidden border border-line bg-ink/50 p-6 sm:p-8"
            >
              ...
              <div className="relative">
                ...
                <p className="mt-9 font-mono text-[10px] uppercase tracking-[0.16em] text-cyan">
                  {activeArea.label} systems
                </p>
                <h3 className="mt-3 max-w-md font-display text-3xl tracking-[-0.05em] text-slate-50">
                ...
                <div className="mt-7 flex flex-wrap gap-2">
                  {activeArea.tools.map((tool) => (
                    <span
                      key={tool}
                      className="border border-line bg-panel/70 px-3 py-2 font-mono text-[10px] text-slate-200"
                    >
                      {tool}
                    </span>
                  ))}
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            {supportingCapabilities.map(({ icon: Icon, label, detail }) => (
              <div key={label} className="border border-line bg-ink/30 p-5">
                <Icon className="h-4 w-4 text-cyan" strokeWidth={1.5} />
                <p className="mt-5 font-display text-lg tracking-[-0.03em] text-slate-100">
                  {label}
                </p>
                <p className="mt-1 font-mono text-xs leading-5 text-mist">
                  {detail}
                </p>
              </div>
            ))}
