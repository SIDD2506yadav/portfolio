    ...
    <section id="about" className="border-y border-line/70 bg-panel/30">
      <Reveal className="mx-auto max-w-6xl px-5 py-20 sm:px-8 md:py-28">
        ...
        <dl className="mt-12 grid grid-cols-3 divide-x divide-line border border-line">
          {metrics.map(({ value, label }) => <div key={label} className="flex flex-col gap-1 bg-ink/30 px-6 py-5"><dt className="font-mono text-[28px] font-medium leading-none tracking-[-0.04em] text-cyan">{value}</dt><dd className="mt-1 font-mono text-[10px] uppercase tracking-[0.1em] text-mist">{label}</dd></div>)}
        </dl>
      </Reveal>
    </section>
  );
