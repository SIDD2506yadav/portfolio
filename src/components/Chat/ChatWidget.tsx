      ...
      <AnimatePresence mode="wait">
        {isOpen ? (
            ...
            <div className="relative border-b border-line">
              ...
              <div className="relative flex items-center justify-between px-5 py-4">
                ...
                <button
                  ...
                  className="flex h-8 w-8 items-center justify-center border border-transparent text-mist transition-colors hover:border-line hover:text-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
            ...
            >
              ...
              <div className="relative space-y-5 px-5 py-5">
                {messages.map((message) => (
                  ...
                  >
                    <div
                      className={`max-w-[88%] border ${message.role === "user" ? "border-cyan/40 bg-cyan/5" : "border-line bg-ink/60"}`}
                    >
                      <div
                        className={`flex items-center justify-between border-b px-3 py-2 ${message.role === "user" ? "border-cyan/20" : "border-line"}`}
                      >
                        <span
                          className={`font-mono text-[9px] uppercase tracking-[0.14em] ${message.role === "user" ? "text-cyan" : "text-mist"}`}
                        >
                ...
                <div className="grid grid-cols-2 gap-2">
                  {SUGGESTIONS.map((suggestion, index) => (
                    <button
                      ...
                      className="group border border-line bg-ink/50 px-3 py-2.5 text-left transition-colors hover:border-cyan/60 hover:bg-cyan/5 disabled:cursor-not-allowed disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan"
                    >
            ...
            >
              <div className="flex items-center border border-line bg-ink transition-colors focus-within:border-cyan/60">
                ...
                <input
                  ...
                  className="caret-cyan min-w-0 flex-1 bg-transparent px-2 py-3.5 text-sm text-slate-100 outline-none placeholder:text-mist/60 disabled:opacity-50"
                />
                <button
                  type="submit"
                  disabled={!input.trim() || isLoading}
                  aria-label="Send message"
                  className="mr-1.5 flex h-9 w-9 shrink-0 items-center justify-center border border-cyan/40 bg-cyan/5 text-cyan transition-colors hover:border-cyan hover:bg-cyan/10 disabled:cursor-not-allowed disabled:opacity-30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan"
                >
                  {isLoading ? (
          ...
          >
            ...
            />
            <div
              className="pointer-events-none absolute -right-10 -top-10 h-24 w-24 rounded-full bg-cyan/10 blur-3xl opacity-0 transition-opacity duration-500 group-hover:opacity-100"
              aria-hidden="true"
            />
            <div className="relative">
              ...
              <div className="flex items-center justify-between gap-3 px-3.5 py-3">
                <div className="flex min-w-0 items-center gap-2.5">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center border border-line bg-ink/60 transition-colors group-hover:border-cyan/40">
