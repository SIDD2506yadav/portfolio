          ...
          <motion.div ref={chatRef} key="chat-window" initial={reducedMotion ? false : { opacity: 0, y: 18, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={reducedMotion ? undefined : { opacity: 0, y: 18, scale: 0.98 }} transition={{ duration: 0.24, ease: "easeOut" }} className="mb-3 flex h-[min(620px,calc(100svh-120px))] w-[calc(100vw-2rem)] flex-col overflow-hidden border border-line bg-panel/95 shadow-2xl shadow-black/50 backdrop-blur-md sm:w-[460px]">
            <div className="relative border-b border-line">
              ...
              <div className="relative flex items-center justify-between px-5 py-4">
                ...
                <button type="button" onClick={() => setIsOpen(false)} aria-label="Close chat" className="flex h-8 w-8 items-center justify-center border border-transparent text-mist transition-colors hover:border-line hover:text-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan"><X className="h-4 w-4" /></button>
              </div>
            ...
            {messages.length === 1 && <div className="border-t border-line px-5 py-4"><div className="mb-3 flex items-center justify-between"><span className="font-mono text-[9px] uppercase tracking-[0.14em] text-mist">Suggested queries</span><span className="font-mono text-[8px] text-mist/60">04</span></div><div className="grid grid-cols-2 gap-2">{SUGGESTIONS.map((suggestion, index) => <button key={suggestion} type="button" disabled={isLoading} onClick={() => handleSuggestion(suggestion)} className="group border border-line bg-ink/50 px-3 py-2.5 text-left transition-colors hover:border-cyan/60 hover:bg-cyan/5 disabled:cursor-not-allowed disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan"><div className="mb-1 flex items-center justify-between"><span className="font-mono text-[8px] text-cyan/70">0{index + 1}</span><ArrowUpRight className="h-3 w-3 text-mist/50 transition-colors group-hover:text-cyan" /></div><span className="font-mono text-[9px] leading-4 text-mist transition-colors group-hover:text-slate-100">{suggestion}</span></button>)}</div></div>}
            <form onSubmit={handleSubmit} className="border-t border-line bg-ink/40 p-4"><div className="flex items-center border border-line bg-ink transition-colors focus-within:border-cyan/60"><span className="pl-3 font-mono text-xs text-cyan">&gt;</span><input ref={inputRef} value={input} onChange={(event) => setInput(event.target.value)} maxLength={300} disabled={isLoading} placeholder="Ask about my work..." aria-label="Chat message" className="caret-cyan min-w-0 flex-1 bg-transparent px-2 py-3.5 text-sm text-slate-100 outline-none placeholder:text-mist/60 disabled:opacity-50" /><button type="submit" disabled={!input.trim() || isLoading} aria-label="Send message" className="mr-1.5 flex h-9 w-9 shrink-0 items-center justify-center border border-cyan/40 bg-cyan/5 text-cyan transition-colors hover:border-cyan hover:bg-cyan/10 disabled:cursor-not-allowed disabled:opacity-30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan">{isLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Send className="h-3.5 w-3.5" />}</button></div><div className="mt-2 flex items-center justify-between"><span className="font-mono text-[8px] uppercase tracking-[0.1em] text-mist/60">AI-powered portfolio assistant</span><span className="font-mono text-[8px] text-mist/60">{input.length}/300</span></div></form>
          </motion.div>
        ) : (
          <motion.button key="chat-trigger" type="button" onClick={() => setIsOpen(true)} initial={reducedMotion ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} whileHover={reducedMotion ? undefined : { y: -2 }} whileTap={reducedMotion ? undefined : { scale: 0.985 }} aria-label="Ask me about my work — open Siddhartha's AI assistant" className="group relative w-[260px] overflow-hidden border border-line bg-panel/95 text-left shadow-2xl shadow-black/40 backdrop-blur-md transition-colors hover:border-cyan/50 sm:w-[300px]">
