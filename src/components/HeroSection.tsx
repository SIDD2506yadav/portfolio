    ...
    <section className="mx-auto grid min-h-[calc(100svh-4rem)] max-w-6xl items-center px-5 py-20 sm:px-8 md:py-28">
      <div className="grid items-end gap-12 lg:grid-cols-[1.25fr_0.75fr]">
        <Reveal>
          ...
          <motion.h1 className="mt-7 max-w-4xl font-display text-[clamp(3.5rem,9vw,6.25rem)] font-medium leading-[0.91] tracking-[-0.075em] text-slate-50" variants={reducedMotion ? undefined : headlineVariants} initial={reducedMotion ? false : "hidden"} animate={reducedMotion ? undefined : "visible"}>
            {headline.map((word, i) => (
              <motion.span key={i} variants={reducedMotion ? undefined : wordVariants} className={`mr-[0.22em] inline-block ${word === "end-to-end" ? "text-cyan" : ""}`}>{word}</motion.span>
            ))}
          </motion.h1>
          <p className="mt-8 max-w-xl text-base leading-7 text-mist sm:text-lg">From polished React interfaces to scalable Node.js services and AI-powered experiences, I turn product requirements into dependable software.</p>
