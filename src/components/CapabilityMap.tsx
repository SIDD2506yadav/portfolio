        ...
        <Reveal delay={0.06}>
          <div className="grid gap-4 lg:grid-cols-[0.85fr_1.15fr]">
            ...
            <motion.div key={activeArea.id} id={`${activeArea.id}-panel`} role="tabpanel" aria-labelledby={`${activeArea.id}-tab`} initial={reducedMotion ? false : { opacity: 0, x: 12 }} animate={reducedMotion ? undefined : { opacity: 1, x: 0 }} transition={{ duration: 0.28 }} className="relative overflow-hidden border border-line bg-ink/50 p-6 sm:p-8">
              ...
              <div className="relative"><ActiveIcon className="h-6 w-6 text-cyan" strokeWidth={1.5} /><p className="mt-9 font-mono text-[10px] uppercase tracking-[0.16em] text-cyan">{activeArea.label} systems</p><h3 className="mt-3 max-w-md font-display text-3xl tracking-[-0.05em] text-slate-50">{activeArea.description}</h3><div className="mt-7 flex flex-wrap gap-2">{activeArea.tools.map((tool) => <span key={tool} className="border border-line bg-panel/70 px-3 py-2 font-mono text-[10px] text-slate-200">{tool}</span>)}</div><div className="mt-8 border-t border-line pt-5"><p className="font-mono text-[10px] uppercase tracking-[0.14em] text-mist">Outcome</p><p className="mt-2 max-w-lg text-sm leading-6 text-slate-200">{activeArea.outcome}</p></div></div>
            </motion.div>
          </div>
